...
指数 P の再較正(2026-06-11, exp37/exp38, reports/14): P=2.0 → **4.0** に更新。
  ...
  ※ **P=4.5 も再審の上で終端棄却**(2026-06-12, exp56/57/58+敵対検証3並列, reports/19)。
    フル期間では +0.48pp 出るが、(a)「IS-argmax=4.5」はグリッド端アーティファクト
    (4.75/5.0 へ延長すると IS は 4.75 が上、[4.0,4.75] は較正ノイズ以下でフラット)、
    (b) IS-emp 基準では argmax≤3.5 で P に単調減少、(c) 事前登録 G3(OOS rob・emp 両改善)
    に不合格(OOS-emp 生CAGR -1.0pp)、(d) フル期間勾配は P5.5 まで単調=reports/14 が
    禁止した「端まで単調」形状そのもの。**P 軸は 4.0 で恒久確定。再訴訟禁止**。
...
DD≤20% の2解釈(--dd-mode で選択)。数値は d1+P=4.0 更新後(括弧内は d0 時代):
  ...
  ※ **mp 軸は M1 粒度で恒久閉鎖**(mp11=exp44、mp9/mp10=exp57)。谷比は用量に単調
    (mp8 1.048 / mp9 1.086 / mp10 1.153 / mp11 1.161)、掛け目込み実効CAGRは mp8 が最大
    (mp9 +17.43% / mp10 +16.54% < mp8 +17.72%)。H4 で見える mp↑ の robust 利得は
    intrabar テール(ボラクラスタの追加玉)への転嫁=レバ偽装。mp8 を維持する。
