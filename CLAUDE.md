...
## 🚀 まず読む(最短経路 — リポジトリ確認はここだけで足りる)

新セッションで「今どこまで進んだか / 何が効いて何が死んだか」を知りたいとき、**全体を探索する必要はない**。以下だけ見れば現状は掴める:

1. **`reports/00_INDEX.md` 冒頭の「📌 現状サマリ」** … 現チャンピオン・天井・恒久閉鎖した軸を10行で。これが知見の**正典(source of truth)**。続く表は番号が大きいほど新しい(歯抜けあり: 11/13 等は別ブランチ)。
2. **本体エンジン** … `fxlab/`(9本)＋`strategies/`＋直下CLI(`evaluate.py`/`leaderboard.py`/`run_backtest.py`/`extract_trades.py`)。**約3,800行だけ**がコア。
3. **ワンショット確認** … `uv run python status.py`(構成・戦略一覧・最新レポート・データ有無を一画面・依存なし・即時)。

⚠️ **`research/` は探索対象から外す**。experiments+outputs で **約36,000行/287ファイル**あるが、これは時系列の**使い捨て検証ログ**。`Grep`/`Glob`/`Explore` でここを掃くと死んだ実験を延々読む羽目になる。**特定の `exp*.py` を指名された時だけ**開く。
⚠️ MEMORY やレポートが**古いファイル名/フラグ/レポート番号を指す**ことがある(別ブランチ由来)。推奨・引用の前に**実在を確認**する。
...
│   └── check_data.py      データ品質チェック(重複/欠損/異常ギャップ)
├── tests/                 pytest(先読み防止・コスト計上・リサンプル等を検証)
...
uv run python scripts/check_data.py               # 品質チェック(重複/欠損/OHLC整合/異常ギャップ)
```

`load()`/`load_m1()` はプロセス内でキャッシュ(同一 pair/tf の再読込は即時)。データ更新後にプロセスを継続する場合は `from fxlab.data import clear_cache; clear_cache()`。**読み込んだ DataFrame は破壊的に書き換えない**(キャッシュ共有のため。必要なら `.copy()`)。
...
uv run python evaluate.py ma_cross --wf                 # ウォークフォワード最適化も実施
uv run python evaluate.py ma_cross --size risk --size-value 0.01   # 1%リスクで現実的に評価
```
...
`--wf`(ウォークフォワード)は単一 IS/OOS より厳しく、**毎回その時点までで再最適化→次の窓で素検証**を n 回繰り返し、「OOSでプラスだった fold の割合」で頑健性を測る(`fxlab.evaluate.walk_forward`)。

**全戦略を横並び比較:**
```bash
uv run python leaderboard.py                  # strategies/ 全戦略を評価→OOS Sharpe降順で一覧
uv run python leaderboard.py --pair USDJPY --tf H4 --size risk --size-value 0.01 --save
```
...
**サイジング(`run`/`sweep`/`evaluate`/各CLIの `size_mode`・`--size`):**
| mode | 内容 |
|---|---|
| `full`(既定) | 余力フル=複利・両建て反転対応(vectorbt 既定)。最も楽観的 |
| `value` | 固定キャッシュ建玉(非複利)。時系列で条件が一定=比較しやすい |
| `amount` | 固定数量(単位) |
| `risk` | 1トレード `size_value`(既定1%)のリスク。ATR×`atr_mult`(既定2)のストップ到達でその損失になるよう建玉。最も実運用的 |

> `full` は「見かけ上すごく勝てる」錯覚を生みやすい(全額複利)。本気で実運用を見るなら `--size risk --size-value 0.01` で再評価する。週末ギャップでストップを飛ばせばリスク%を超える損失も起こりうる(=現実)。
...
```bash
uv run pytest -q          # 全テスト(合成データで高速・データDL不要)
```

`tests/` がエンジンの正しさを守る安全網:**先読み(look-ahead)していないか / コストがリターンを下げるか / リサンプル集約規則 / sweep のランク / サイジング各モード / リスク%の損失上限 / トレード抽出**。

→ **`fxlab/` のコア(backtest・data・evaluate・trades)を変更したら必ず `uv run pytest` を通す。** 落ちたら結論を出す前に直す。新しい不変条件を足したらテストも追加。

## 9. 規約・注意

- **CLIツールは高速版を使う(下表)。** 未導入環境はまとめて `brew install ripgrep fd duckdb csvlens dust bat`。

  | 用途 | ツール | このリポでの使い方(例) |
  |---|---|---|
  | コード検索(grep) | **`rg`** | `rg "generate_signals" strategies/` / 型指定 `rg -t py "PARAM_GRID"`。再帰・`.gitignore`尊重が既定で `data/`・`results/` を自動除外して速い |
  | ファイル検索(find) | **`fd`** | `fd -e parquet data/` / `fd eval_ results/` / `fd -e py . strategies` |
  | データ即SQL照会 | **`duckdb`** | Python起動なしで parquet/CSV を直接クエリ。まず `duckdb -c "DESCRIBE SELECT * FROM 'data/raw/EURUSD_M1.parquet'"` で列確認 → `duckdb -c "SELECT count(*), min(close), max(close) FROM 'data/raw/EURUSD_M1.parquet'"`。結果CSVの集計も `duckdb -c "SELECT * FROM 'results/eval_x.csv' ORDER BY sharpe DESC LIMIT 10"` |
  | CSV閲覧(TUI) | **`csvlens`** | `csvlens results/xxx.csv`(列揃え・検索・フィルタ付きで `results/`・`research/outputs/` のCSVを見る) |
  | 容量確認(du) | **`dust`** | `dust data/raw`(gitignore外の巨大M1 parquetがどれだけ容量を食うか一目) |
  | ファイル表示(cat) | **`bat`** | `bat strategies/ma_cross.py`(シンタックスハイライト+行番号。差分やコード確認に) |

  > データの中身を確かめたいだけなら **`duckdb` が最速**(pandas読込より起動が軽い)。本格的な計算・バックテストは従来どおり `uv run python`(§5)。
- 実行は常に `uv run`。`pip install` 単体は使わない(`uv add` / dev は `uv add --dev`)。
- 時刻は全て **UTC**。`data/`・`results/` は git 管理外。
- ライブ発注・送金は**しない**(検証専用環境)。
- コア API を変えたら §8 の `pytest` と §5 の例で動作確認する。
- 新しい手法を足すたびに `strategies/` に 1 ファイル追加し、`evaluate.py`(必要なら `--wf`)まで回して結論づける。
- 読み込んだ価格 DataFrame は破壊的に変更しない(キャッシュ共有)。
